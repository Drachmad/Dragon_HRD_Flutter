...
class model_kik_packing {
  String baseUrl = base_url;

  ///paginate
  Future count_kik_packing_paginate(String key_cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/count_kik_packing_paginate"),
      body: {"cari": key_cari},
    );
  ...
  /// paginate
  Future<List> data_kik_packing_paginate(
      String cari, int paramoffset, int paramlimit) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/kik_packing_paginate"),
      body: {
        "cari": cari,
  ...
  Future<List> insert_kik_packing(Map data_insert) async {
    try {
      ///DATA HEADER
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/tambah_header_kik_packing"),
        body: {
      ...
      for (int i = 0; i < data_detail.length; i++) {
        await http.post(
          Uri.parse("${baseUrl}:3000/tambah_detail_kik_packing"),
          body: {
            "no_bukti": data_insert['no_bukti'].toString(),
  ...
  Future<List> update_kik_packing(Map data_insert) async {
    try {
      await http.post(
      ...
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/edit_header_kik_packing"),
        body: {
          "no_bukti": data_insert['no_bukti'].toString(),
          "kd_bag": data_insert['kd_bag'].toString(),
          "nm_bag": data_insert['nm_bag'].toString(),
  ...
  Future<List> urut_nobukti(String per) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/urut_nobukti_kik_packing"),
      body: {"per": per},
    );
    var results2 = json.decode(response.body);
    return results2['data'].toList();
  ...
  ///SELECT DETAIL
  Future<List> select_kik_packing_detail(
  ...
  Future delete_kik_packing(String no_bukti) async {
    try {
      await http.post(
        Uri.parse("${baseUrl}:3000/hapus_kik_packing"),
