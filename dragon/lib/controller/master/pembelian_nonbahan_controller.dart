...
import 'package:dragon/model/master/model_pembelian_nonbahan.dart';
import 'package:dragon/view/base_widget/toast.dart';

class Pembelian_NonbahanController with ChangeNotifier {
  List data_pembelianNonbahanList = [];
  bool proses = false;
  ...
  model_pembelian_nonbahan m_pembelian_nonbahan = model_pembelian_nonbahan();
  ...
  Future<void> selectDataPaginate(bool reload, String cari) async {
    ...
    data_pembelianNonbahanList = await m_pembelian_nonbahan
        .data_pembelian_nonbahan_paginate(cari, offset, limit);
    var count =
        await m_pembelian_nonbahan.count_pembelian_nonbahan_paginate(cari);
  ...
  void selectData(String cari) async {
    ...
    page_index = 0;
    // await selectDataPaginate(true);
    data_pembelianNonbahanList = await model_pembelian_nonbahan()
        .data_pembelian_nonbahan_paginate(cari, offset, limit);
    var count =
        await m_pembelian_nonbahan.count_pembelian_nonbahan_paginate(cari);
    totalNotaTerima = int.tryParse(count[0]['COUNT(*)'].toString()) ?? 0;
  ...
  Future<bool> tambah_nonmesin() async {
    if (KD_BHNController.text.isNotEmpty) {
      ...
      await model_pembelian_nonbahan().insert_data_nonmesin(data_insert);
  ...
  Future<bool> edit_nonmesin(var id) async {
    if (AKTIFController.text.isNotEmpty) {
      ...
      data_insert['AKTIF'] = AKTIFController.text;
      // print(data_insert);
      await model_pembelian_nonbahan().update_data_nonmesin_by_id(data_insert);
  ...
  Future<bool> hapus_pembelianNonbahan(var data) async {
    await model_pembelian_nonbahan()
        .delete_nonmesin_byID(data['NO_ID'].toString());
    initData();
  }
}
