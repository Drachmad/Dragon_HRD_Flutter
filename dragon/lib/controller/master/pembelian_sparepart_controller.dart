...
import 'package:dragon/model/master/model_pembelian_sparepart.dart';
import 'package:dragon/view/base_widget/toast.dart';

class Pembelian_SparepartController with ChangeNotifier {
  List data_pembelianSparepartList = [];
  bool proses = false;
  ...
  model_pembelian_sparepart m_pembelian_sparepart = model_pembelian_sparepart();
  ...
  Future<void> selectDataPaginate(bool reload, String cari) async {
    ...
    data_pembelianSparepartList = await m_pembelian_sparepart
        .data_pembelian_sparepart_paginate(cari, offset, limit);
    var count =
        await m_pembelian_sparepart.count_pembelian_sparepart_paginate(cari);
  ...
  void selectData(String cari) async {
    ...
    page_index = 0;
    // await selectDataPaginate(true);
    data_pembelianSparepartList = await model_pembelian_sparepart()
        .data_pembelian_sparepart_paginate(cari, offset, limit);
    var count =
        await m_pembelian_sparepart.count_pembelian_sparepart_paginate(cari);
    totalNotaTerima = int.tryParse(count[0]['COUNT(*)'].toString()) ?? 0;
  ...
  Future<bool> tambah_sparepart() async {
    if (KD_BHNController.text.isNotEmpty) {
      ...
      await model_pembelian_sparepart().insert_data_sparepart(data_insert);
  ...
  Future<bool> edit_sparepart(var id) async {
    if (AKTIFController.text.isNotEmpty) {
      ...
      data_insert['AKTIF'] = AKTIFController.text;
      // print(data_insert);
      await model_pembelian_sparepart()
          .update_data_sparepart_by_id(data_insert);
  ...
  Future<bool> hapus_sparepart(var data) async {
    await model_pembelian_sparepart()
        .delete_sparepart_byID(data['NO_ID'].toString());
    initData();
  }
}
