...
class KikJahitController with ChangeNotifier {
  model_kik_jahit m_kikjahit = model_kik_jahit();
  ...
  Future<void> initData_editKikJahit(var data_edit) async {
    ...
    List data_lama = await m_kikjahit.select_kik_jahit_detail(
  ...
  Future<bool> saveKikJahit() async {
    ...
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        ...
        var data_ready = await m_kikjahit.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikjahit.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikjahit.insert_kik_jahit(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikJahit() async {
    ...
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikjahit.update_kik_jahit(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikJahit(String no_bukti) async {
    try {
      var delete = await m_kikjahit.delete_kik_jahit(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
  ...
  Future<List> baca_tabeld() async {
    ...
    for (int i = 0; i < data_kik_keranjang.length; i++) {
      ...
      int upah = data_kik_keranjang[i].upah;
  ...
  Future<void> report_tes() async {
    await m_kikjahit.report_tes();
  }
}
