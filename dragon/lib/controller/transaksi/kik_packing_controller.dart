...
import 'package:dragon/model/transaksi/model_kik_packing.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikPackingController with ChangeNotifier {
  model_kik_packing m_kikpacking = model_kik_packing();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_packing_list = [];
  bool proses = false;
  ...
  model_kik_packing m_kik_packing = model_kik_packing();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_packing_list =
        await m_kik_packing.data_kik_packing_paginate(cari, offset, limit);
    var count = await m_kik_packing.count_kik_packing_paginate(cari);
  ...
  Future<void> initData_addKikPacking() async {
  ...
  Future<void> initData_editKikPacking(var data_edit) async {
    ...
    List data_lama = await m_kikpacking.select_kik_packing_detail(
  ...
  /// data header
  Future<bool> saveKikPacking() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikpacking.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikpacking.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikpacking.insert_kik_packing(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikPacking() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikpacking.update_kik_packing(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikPacking(String no_bukti) async {
    try {
      var delete = await m_kikpacking.delete_kik_packing(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
