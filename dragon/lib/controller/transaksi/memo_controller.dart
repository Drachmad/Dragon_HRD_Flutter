...
import 'package:dragon/model/transaksi/model_memo.dart';
...
class MemoController with ChangeNotifier {
  ...
  Future<void> select_data() async {
    data_order_penjualan_list = await m_order.select_memo(
  ...
  Future<void> initData_addMemo() async {
    ...
    sumTotal = 0;
    await m_order
        .count_memo(format_created_at2.format(DateTime.now()))
        .then((value) {
      if (value != null) {
        if (value.length > 0) {
    ...
    if (nobuktiController.text.isNotEmpty) {
      if (data_account_keranjang.length > 0) {
        ...
        var data_ready = await m_order.get_no_bukti(
            nobuktiController.text, "NO_BUKTI", "memo");
