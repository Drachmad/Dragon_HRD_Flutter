...
import 'package:dragon/model/transaksi/model_kik_plong.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikPlongController with ChangeNotifier {
  model_kik_plong m_kikplong = model_kik_plong();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_plong_list = [];
  bool proses = false;
  ...
  model_kik_plong m_kik_plong = model_kik_plong();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_plong_list =
        await m_kik_plong.data_kik_plong_paginate(cari, offset, limit);
    var count = await m_kik_plong.count_kik_plong_paginate(cari);
  ...
  Future<void> initData_addKikPlong() async {
  ...
  Future<void> initData_editKikPlong(var data_edit) async {
    ...
    List data_lama = await m_kikplong.select_kik_plong_detail(
  ...
  /// data header
  Future<bool> saveKikPlong() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikplong.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikplong.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikplong.insert_kik_plong(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikPlong() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikplong.update_kik_plong(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikPlong(String no_bukti) async {
    try {
      var delete = await m_kikplong.delete_kik_plong(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
