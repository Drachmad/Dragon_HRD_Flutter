...
import 'package:dragon/model/transaksi/model_bankmasuk.dart';
...
class BankmasukController with ChangeNotifier {
  ...
  Future<void> select_data() async {
    data_order_penjualan_list = await m_order.select_bankmasuk(
  ...
  Future<void> initData_addBankmasuk() async {
    ...
    sumTotal = 0;
    await m_order
        .countBankMasuk(format_created_at2.format(DateTime.now()))
        .then((value) {
      if (value != null) {
        if (value.length > 0) {
  ...
  Future<bool> deleteBankmasuk(String no_bukti) async {
    try {
      var delete = await m_order.delete_bankmasuk(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
