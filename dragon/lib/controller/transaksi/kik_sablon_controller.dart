...
import 'package:dragon/model/transaksi/model_kik_sablon.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikSablonController with ChangeNotifier {
  model_kik_sablon m_kiksablon = model_kik_sablon();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_sablon_list = [];
  bool proses = false;
  ...
  model_kik_sablon m_kik_sablon = model_kik_sablon();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_sablon_list =
        await m_kik_sablon.data_kik_sablon_paginate(cari, offset, limit);
    var count = await m_kik_sablon.count_kik_sablon_paginate(cari);
  ...
  Future<void> initData_addKikSablon() async {
  ...
  Future<void> initData_editKikSablon(var data_edit) async {
    ...
    List data_lama = await m_kiksablon.select_kik_sablon_detail(
  ...
  /// data header
  Future<bool> saveKikSablon() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kiksablon.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kiksablon.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kiksablon.insert_kik_sablon(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikSablon() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kiksablon.update_kik_sablon(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikSablon(String no_bukti) async {
    try {
      var delete = await m_kiksablon.delete_kik_sablon(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
