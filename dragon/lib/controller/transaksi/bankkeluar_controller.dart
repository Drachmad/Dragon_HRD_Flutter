...
import 'package:dragon/model/transaksi/model_bankkeluar.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class BankkeluarController with ChangeNotifier {
  model_bankkeluar m_order = model_bankkeluar();
  ...
  Future<void> select_data() async {
    data_order_penjualan_list = await m_order.select_bankkeluar(
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      List data_account = await m_order.select_bankkeluar_detail(
          data_order_penjualan_list[index_terpilih]['no_bukti'],
          "NO_BUKTI",
          "bank");
  ...
  bool status_bankkeluar = true;

  Future<void> initData_addBankkeluar() async {
    ...
    sumTotal = 0;
    await m_order
        .countBankKeluar(format_created_at2.format(DateTime.now()))
        .then((value) {
      if (value != null) {
        if (value.length > 0) {
        ...
        nobuktiController.text =
            "BBK${format_no_bukti.format(DateTime.now())}B-${no_urut + 1}";
      }
    });
  ...
  Future<void> initData_editBankkeluar(var data_edit) async {
    ...
    // status_bankkeluar = data_edit['POSTED'] == 1 ? true : false;
    List data_lama = await m_order.select_bankkeluar_detail(
  ...
  /// data header
  Future<bool> saveBankkeluar() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_account_keranjang.length > 0) {
        ...
        if (data_ready.length > 0) {
          ...
          await m_order.insert_bankkeluar(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editBankkeluar() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_account_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['keterangan'] = keteranganController.text;
        obj['tabeld'] = await baca_tabeld();
        await m_order.update_bankkeluar(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteBankkeluar(String no_bukti) async {
    try {
      var delete = await m_order.delete_bankkeluar(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
