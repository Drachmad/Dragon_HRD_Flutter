...
class KikStrongController with ChangeNotifier {
  ...
  model_kik_strong m_kik_strong = model_kik_strong();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_strong_list =
        await m_kik_strong.data_kik_strong_paginate(cari, offset, limit);
    var count = await m_kik_strong.count_kik_strong_paginate(cari);
  ...
  Future<void> initData_addKikStrong() async {
  ...
  Future<void> initData_editKikStrong(var data_edit) async {
    ...
    List data_lama = await m_kikstrong.select_kik_strong_detail(
  ...
  /// data header
  Future<bool> saveKikStrong() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikstrong.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikstrong.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikstrong.insert_kik_strong(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikStrong() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikstrong.update_kik_strong(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikStrong(String no_bukti) async {
    try {
      var delete = await m_kikstrong.delete_kik_strong(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
