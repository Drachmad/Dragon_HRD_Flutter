...
import 'package:dragon/model/transaksi/model_kik_compound.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikCompoundController with ChangeNotifier {
  model_kik_compound m_kikcompound = model_kik_compound();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_compound_list = [];
  bool proses = false;
  ...
  model_kik_compound m_kik_compound = model_kik_compound();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_compound_list =
        await m_kik_compound.data_kik_compound_paginate(cari, offset, limit);
    var count = await m_kik_compound.count_kik_compound_paginate(cari);
  ...
  Future<void> initData_addKikCompound() async {
  ...
  Future<void> initData_editKikCompound(var data_edit) async {
    ...
    List data_lama = await m_kikcompound.select_kik_compound_detail(
  ...
  /// data header
  Future<bool> saveKikCompound() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikcompound.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikcompound.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikcompound.insert_kik_compound(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikCompound() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikcompound.update_kik_compound(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikCompound(String no_bukti) async {
    try {
      var delete = await m_kikcompound.delete_kik_compound(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
