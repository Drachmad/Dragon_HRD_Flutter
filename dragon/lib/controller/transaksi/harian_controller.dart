...
class HarianController with ChangeNotifier {
  model_harian m_order = model_harian();
  ...
  model_harian m_harian = model_harian();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_harian_list = await m_harian.data_harian_paginate(cari, offset, limit);
    var count = await m_harian.count_harian_paginate(cari);
  ...
  //   data_harian_list = await m_order.data_harian_paginate(
  ...
  //     for (int i = 0; i < data_harian_list.length; i++) {
  //       Map<String, dynamic> isi_map = new Map<String, dynamic>();
  //       isi_map['a'] = data_harian_list[i]['tanggal'];
  //       isi_map['b'] = data_harian_list[i]['no_bukti'];
  //       isi_map['c'] = data_harian_list[i]['sales'];
  //       isi_map['d'] = data_harian_list[i]['customer'];
  //       isi_map['e'] = data_harian_list[i]['keterangan'];
  //       isi_map['f'] = data_harian_list[i]['total_qty'];
  //       isi_map['g'] = data_harian_list[i]['total_so'];
  //       if (data_harian_list[i]['status'] == 1) {
  //         isi_map['h'] = "Diterima";
  //       } else {
  ...
  //     map_transaksi['a'] = data_harian_list[index_terpilih]['tanggal'];
  //     map_transaksi['b'] = data_harian_list[index_terpilih]['no_bukti'];
  //     map_transaksi['c'] = data_harian_list[index_terpilih]['sales'];
  //     map_transaksi['d'] = data_harian_list[index_terpilih]['customer'];
  //     map_transaksi['e'] = data_harian_list[index_terpilih]['keterangan'];
  //     if (data_harian_list[index_terpilih]['status'] == 1) {
  ...
  //     List data_pegawai = await m_order.select_harian_detail(
  //         data_harian_list[index_terpilih]['no_bukti'], "no_bukti", "hrd_absen");
  ...
  //     footer_excel.add(data_harian_list[index_terpilih]['total_qty']);
  //     footer_excel.add("Total");
  //     footer_excel.add(data_harian_list[index_terpilih]['total_so']);
  //     String judul =
  //         "Invoice Order Penjualan (${data_harian_list[index_terpilih]['no_bukti']})";
  ...
  //   //     .proses_print(data_harian_list[index_terpilih], data_pegawai);
  // }
  ...
  Future<void> initData_editHarian(var data_edit) async {
    ...
    // status_harian = data_edit['POSTED'] == 1 ? true : false;
    List data_lama = await m_order.select_harian_detail(
  ...
  Future<bool> editHarian() async {
    ...
    if (kd_bagController.text.isNotEmpty) {
      if (data_pegawai_keranjang.length > 0) {
        ...
        obj['notes'] = notesController.text;
        obj['tabeld'] = await baca_tabeld();
        await m_order.update_harian(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteHarian(String no_bukti) async {
    try {
      var delete = await m_order.delete_harian(no_bukti);
      await selectDataPaginate(true, '');
      return true;
    } catch (e) {
      return false;
