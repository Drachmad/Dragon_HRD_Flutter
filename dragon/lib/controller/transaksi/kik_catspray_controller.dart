...
import 'package:dragon/model/transaksi/model_kik_catspray.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikCatSprayController with ChangeNotifier {
  model_kik_catspray m_kikcatspray = model_kik_catspray();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_catspray_list = [];
  bool proses = false;
  ...
  model_kik_catspray m_kik_catspray = model_kik_catspray();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_catspray_list =
        await m_kik_catspray.data_kik_catspray_paginate(cari, offset, limit);
    var count = await m_kik_catspray.count_kik_catspray_paginate(cari);
  ...
  Future<void> initData_addKikCatSpray() async {
  ...
  Future<void> initData_editKikCatSpray(var data_edit) async {
    ...
    List data_lama = await m_kikcatspray.select_kik_catspray_detail(
  ...
  /// data header
  Future<bool> saveKikCatSpray() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikcatspray.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikcatspray.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikcatspray.insert_kik_catspray(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikCatSpray() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikcatspray.update_kik_catspray(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikCatSpray(String no_bukti) async {
    try {
      var delete = await m_kikcatspray.delete_kik_catspray(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
