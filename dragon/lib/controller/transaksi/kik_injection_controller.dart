...
import 'package:dragon/model/transaksi/model_kik_injection.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikInjectionController with ChangeNotifier {
  model_kik_injection m_kikinjection = model_kik_injection();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_injection_list = [];
  bool proses = false;
  ...
  model_kik_injection m_kik_injection = model_kik_injection();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_injection_list =
        await m_kik_injection.data_kik_injection_paginate(cari, offset, limit);
    var count = await m_kik_injection.count_kik_injection_paginate(cari);
  ...
  Future<void> initData_addKikInjection() async {
  ...
  Future<void> initData_editKikInjection(var data_edit) async {
    ...
    List data_lama = await m_kikinjection.select_kik_injection_detail(
  ...
  /// data header
  Future<bool> saveKikInjection() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikinjection.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikinjection.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikinjection.insert_kik_injection(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikInjection() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikinjection.update_kik_injection(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikInjection(String no_bukti) async {
    try {
      var delete = await m_kikinjection.delete_kik_injection(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
