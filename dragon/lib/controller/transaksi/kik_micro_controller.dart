...
import 'package:dragon/model/transaksi/model_kik_micro.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikMicroController with ChangeNotifier {
  model_kik_micro m_kikmicro = model_kik_micro();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_micro_list = [];
  bool proses = false;
  ...
  model_kik_micro m_kik_micro = model_kik_micro();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_micro_list =
        await m_kik_micro.data_kik_micro_paginate(cari, offset, limit);
    var count = await m_kik_micro.count_kik_micro_paginate(cari);
  ...
  Future<void> initData_addKikMicro() async {
  ...
  Future<void> initData_editKikMicro(var data_edit) async {
    ...
    List data_lama = await m_kikmicro.select_kik_micro_detail(
  ...
  /// data header
  Future<bool> saveKikMicro() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikmicro.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikmicro.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikmicro.insert_kik_micro(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikMicro() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikmicro.update_kik_micro(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikMicro(String no_bukti) async {
    try {
      var delete = await m_kikmicro.delete_kik_micro(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
