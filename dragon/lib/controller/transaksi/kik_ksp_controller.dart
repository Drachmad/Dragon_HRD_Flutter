...
import 'package:dragon/model/transaksi/model_kik_ksp.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikKspController with ChangeNotifier {
  model_kik_ksp m_kikksp = model_kik_ksp();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_ksp_list = [];
  bool proses = false;
  ...
  model_kik_ksp m_kik_ksp = model_kik_ksp();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_ksp_list =
        await m_kik_ksp.data_kik_ksp_paginate(cari, offset, limit);
    var count = await m_kik_ksp.count_kik_ksp_paginate(cari);
  ...
  Future<void> initData_addKikKsp() async {
  ...
  Future<void> initData_editKikKsp(var data_edit) async {
    ...
    List data_lama = await m_kikksp.select_kik_ksp_detail(
  ...
  /// data header
  Future<bool> saveKikKsp() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikksp.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikksp.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikksp.insert_kik_ksp(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikKsp() async {
    ...
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikksp.update_kik_ksp(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikKsp(String no_bukti) async {
    try {
      var delete = await m_kikksp.delete_kik_ksp(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
