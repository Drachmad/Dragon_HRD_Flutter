...
import 'package:dragon/model/transaksi/model_kik_assembling.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikAssemblingController with ChangeNotifier {
  model_kik_assembling m_kikassembling = model_kik_assembling();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_assembling_list = [];
  bool proses = false;
  ...
  model_kik_assembling m_kik_assembling = model_kik_assembling();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_assembling_list = await m_kik_assembling
        .data_kik_assembling_paginate(cari, offset, limit);
    var count = await m_kik_assembling.count_kik_assembling_paginate(cari);
  ...
  Future<void> initData_addKikAssembling() async {
  ...
  Future<void> initData_editKikAssembling(var data_edit) async {
    ...
    List data_lama = await m_kikassembling.select_kik_assembling_detail(
  ...
  /// data header
  Future<bool> saveKikAssembling() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikassembling.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikassembling.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikassembling.insert_kik_assembling(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikAssembling() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikassembling.update_kik_assembling(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikAssembling(String no_bukti) async {
    try {
      var delete = await m_kikassembling.delete_kik_assembling(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
