...
import 'package:dragon/model/transaksi/model_kik_dr2.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikDR2Controller with ChangeNotifier {
  model_kik_dr2 m_kikdr2 = model_kik_dr2();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_dr2_list = [];
  bool proses = false;
  ...
  model_kik_dr2 m_kik_dr2 = model_kik_dr2();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_dr2_list =
        await m_kik_dr2.data_kik_dr2_paginate(cari, offset, limit);
    var count = await m_kik_dr2.count_kik_dr2_paginate(cari);
  ...
  Future<void> initData_addKikDR2() async {
  ...
  Future<void> initData_editKikDR2(var data_edit) async {
    ...
    List data_lama = await m_kikdr2.select_kik_dr2_detail(
  ...
  /// data header
  Future<bool> saveKikDR2() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikdr2.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikdr2.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikdr2.insert_kik_dr2(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikDR2() async {
    ...
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikdr2.update_kik_dr2(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikDR2(String no_bukti) async {
    try {
      var delete = await m_kikdr2.delete_kik_dr2(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
