...
import 'package:dragon/model/transaksi/model_kik_flocking.dart';
import 'package:dragon/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class KikFlockingController with ChangeNotifier {
  model_kik_flocking m_kikflocking = model_kik_flocking();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_kik_flocking_list = [];
  bool proses = false;
  ...
  model_kik_flocking m_kik_flocking = model_kik_flocking();
  ...
  Future<void> selectDataPaginate(bool reload, cari) async {
    ...
    data_kik_flocking_list =
        await m_kik_flocking.data_kik_flocking_paginate(cari, offset, limit);
    var count = await m_kik_flocking.count_kik_flocking_paginate(cari);
  ...
  Future<void> initData_addKikFlocking() async {
  ...
  Future<void> initData_editKikFlocking(var data_edit) async {
    ...
    List data_lama = await m_kikflocking.select_kik_flocking_detail(
  ...
  /// data header
  Future<bool> saveKikFlocking() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_kikflocking.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_kikflocking.urut_nobukti(per).then((value) {
            if (value != null) {
              if (value.length > 0) {
          ...
          await m_kikflocking.insert_kik_flocking(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editKikFlocking() async {
    hitungSubTotal();
    if (kd_bagController.text.isNotEmpty) {
      if (data_kik_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_kikflocking.update_kik_flocking(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteKikFlocking(String no_bukti) async {
    try {
      var delete = await m_kikflocking.delete_kik_flocking(no_bukti);
      await selectDataPaginate(true, searchController.text);
      return true;
    } catch (e) {
      return false;
